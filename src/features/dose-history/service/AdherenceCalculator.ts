import type { DoseHistory, AdherenceStats, DoseHistoryFilter } from '../model';
...
export class AdherenceCalculator {
    ...
    static calculateStats(doseHistory: DoseHistory[]): AdherenceStats {
        ...
        const lateHistory = doseHistory.filter(d =>
            (d.status === 'late' || d.status === 'taken') && d.lateMinutes && d.lateMinutes > 0
        );

        const averageLateMinutes = lateHistory.length > 0
    ...
    static calculateByPeriod(
        ...
        for (let i = 0; i < 4; i++) { // Últimas 4 semanas
     ...
     */
    static getAdherenceLevel(adherencePercentage: number): 'excellent' | 'good' | 'moderate' | 'poor' {
